				  ...
				  struct affinity_context *ctx)
{
	...
	if ((p->flags & PF_PERF_CRITICAL) && ctx->new_mask != cpu_lp_mask &&
	    ctx->new_mask != cpu_perf_mask && ctx->new_mask != cpu_prime_mask)
		return -EINVAL;
...
static void try_to_wake_up_local(struct task_struct *p, struct rq_flags *rf)
{
	...
	if (!(READ_ONCE(p->__state) & TASK_NORMAL))
		goto out;
...
void scheduler_tick(void)
{
	...
	struct task_struct *curr = rq->curr;
	struct rq_flags rf;
	unsigned int flag = 0, thermal_pressure;
...
void show_state_filter_single(unsigned int state_filter)
{
	struct task_struct *g, *p;
