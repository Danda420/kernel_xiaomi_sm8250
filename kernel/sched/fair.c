...
update_stats_wait_end_fair(struct cfs_rq *cfs_rq, struct sched_entity *se)
{
	 ...
	 * maybe already in the runqueue, the stats.wait_start
...
update_stats_dequeue_fair(struct cfs_rq *cfs_rq, struct sched_entity *se, int flags)
{
	...
	if ((flags & DEQUEUE_SLEEP) && entity_is_task(se)) {
		...
		if (tsk->state & TASK_INTERRUPTIBLE)
			__schedstat_set(se->stats.sleep_start,
				      rq_clock(rq_of(cfs_rq)));
		if (tsk->state & TASK_UNINTERRUPTIBLE)
			__schedstat_set(se->stats.block_start,
				      rq_clock(rq_of(cfs_rq)));
	}
}
			...
			bool boosted)
{
	...
	if (boosted || shallowest_idle_cstate <= 0) {
		*target_cpu = -1;
		return;
	}
...
static int select_idle_cpu(struct task_struct *p, struct sched_domain *sd, int target)
{
	...
	for_each_cpu_wrap(cpu, sched_domain_span(sd), target + 1) {
		...
		if (smt) {
			i = select_idle_core(p, cpu, sched_domain_span(sd), &idle_cpu);
	       ...
	       struct cpumask *pd_cpus, struct task_struct *p, int dst_cpu)
{
	...
	return em_pd_energy(pd->em_pd, max_util, busy_time, eenv->cpu_cap);
}
...
static void __set_next_task_fair(struct rq *rq, struct task_struct *p);
...
static inline void update_sg_lb_stats(struct lb_env *env,
				      ...
				      int *sg_status)
{
	int local_group = group == sds->local;
