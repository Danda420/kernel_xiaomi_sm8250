...
static void binder_free_proc(struct binder_proc *proc)
{
	...
	kmem_cache_free(binder_eproc_pool, proc_wrapper(proc));
}
static void binder_free_thread(struct binder_thread *thread)
{
...
static int binder_open(struct inode *nodp, struct file *filp)
{
	...
	proc_wrap = kmem_cache_zalloc(binder_eproc_pool, GFP_KERNEL);
	if (proc_wrap == NULL)
		return -ENOMEM;
	proc = &proc_wrap->proc;
...
static int __init binder_create_pools(void)
{
	...
	binder_eproc_pool = KMEM_CACHE(binder_proc_wrap, SLAB_HWCACHE_ALIGN);
