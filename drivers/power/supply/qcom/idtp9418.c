			  ...
			  const struct i2c_device_id *id)
{
	...
	if (gpio_is_valid(di->dt_props.hall3_gpio)) {
		...
		if (!hall3_val) {
			...
			di->hall3_online = 1;
			queue_delayed_work(system_power_efficient_wq, &di->hall3_irq_work, msecs_to_jiffies(5000));
		}
	}
	else
	...
	if (gpio_is_valid(di->dt_props.hall4_gpio)) {
		...
		if (!hall4_val) {
			dev_info(di->dev, "pen online, start reverse charge\n");
			di->hall4_online = 1;
			queue_delayed_work(system_power_efficient_wq, &di->hall4_irq_work, msecs_to_jiffies(5000));
		}
	}
	else
