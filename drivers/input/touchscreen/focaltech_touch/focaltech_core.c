...
static int last_orientation_value = 0;
/*****************************************************************************
* irq_num is Log print mode variable control for easy debugging
*****************************************************************************/
static int irq_num;
...
int fts_reset_proc(int hdelayms)
{
	...
	while(read_time){
		ret = fts_i2c_read_reg(client, FTS_REG_CHIP_ID, &chip_id[0]);
		ret = fts_i2c_read_reg(client, FTS_REG_CHIP_ID2, &chip_id[1]);
		FTS_INFO("chip_id[0] = 0x%x chip_id[1] = 0x%x\n",chip_id[0], chip_id[1]);
		if( chip_id[0] == 0x54 && chip_id[1] == 0x52)
			break;
		mdelay(15);
		read_time--;
	}
...
static int fts_input_report_b(struct fts_ts_data *data)
{
	...
	for (i = 0; i < data->touch_point; i++) {
		...
		if (EVENT_DOWN(events[i].flag)) {
			...
			if (!data->fod_finger_skip && fod_overlap_aera == 100)
			...
			if (irq_num == 1)
				FTS_INFO("Report_0x152 resume DOWN report_area %d ", fod_overlap_aera);
	...
	if (ret < 0) {
		...
		case 0x26:
			if (buf[9] == 0) {
				...
				if (!data->suspended) {
					if (irq_num == 1)
						FTS_INFO("Touch is not in suspend state, report x,y value by touch nomal report\n");
					mutex_unlock(&data->report_mutex);
					return -EINVAL;
				}
				...
				if (!data->fod_finger_skip && data->finger_in_fod) {
					...
					input_sync(data->input_dev);
					if (irq_num == 1)
						FTS_INFO("Report_0x152 suspend DOWN report_area %d sucess for miui", fod_overlap_aera);
				}
				mutex_unlock(&data->report_mutex);
			} else {
...
static int fts_palm_sensor_write(int value)
{
	...
	if (!fts_data->suspended) {
		FTS_ERROR("-----%s-----suspended----value=%d\n",__func__,value);
		ret = fts_palm_enable(fts_data, value);
		if (!ret)
...
static void fts_update_touchmode_data(int mode)
{
	...
	FTS_ERROR("%s---mode=%d,temp_value=%d\n",__func__,mode,temp_value);
	switch (mode) {
	...
	case Touch_Panel_Orientation:
			last_orientation_value = temp_value;
	...
	case Touch_Edge_Filter:
			if (fts_data->gamemode_enabled) {
				...
				FTS_ERROR("%s---last_orientation_value=%d\n",__func__,last_orientation_value);
				if(last_orientation_value == 1){
					ret = fts_i2c_write_reg(fts_data->client, FTS_REG_EDGE_FILTER_ORIENTATION, 1);
					if (ret < 0)
						FTS_ERROR("write last_orientation_value error, ret=%d\n", ret);
				}
				if(last_orientation_value == 3){
					ret = fts_i2c_write_reg(fts_data->client, FTS_REG_EDGE_FILTER_ORIENTATION, 2);
					if (ret < 0)
						FTS_ERROR("write last_orientation_value error, ret=%d\n", ret);
				}
...
static int fts_get_dt_coords(struct device *dev, char *name, struct fts_ts_platform_data *pdata)
{
	...
	FTS_INFO("lockdown_info_addr = 0x%x\n",pdata->lockdown_info_addr);
	return 0;
}
...
static int fts_parse_dt(struct device *dev, struct fts_ts_platform_data *pdata)
{
	...
	if (pdata->vdd_gpio < 0)
		FTS_ERROR("Invalid vdd-gpio in dt: %d", pdata->vdd_gpio);
...
static ssize_t fts_tp_debug_read(struct file *file, char __user *buf, size_t count, loff_t *pos)
{
	return 0;
}


ssize_t fts_tp_debug_write(struct file *file, const char __user *buf, size_t count, loff_t *pos)
{
	char tmp[2];
...
int fts_tp_debug_release(struct inode *inode, struct file *file)
{
	return 0;
}

int fts_tp_debug_open(struct inode *inode, struct file *file)
{
	return 0;
}
...
static ssize_t fts_fod_test_write(struct device *dev,
					struct device_attribute *attr,
					const char *buf, size_t count)
{
	int value = 0;
...
static int fts_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	ts_data->tp_debug_en_proc = proc_create("tp_debug_en", 0644, NULL, &fts_tp_debug_ops);
